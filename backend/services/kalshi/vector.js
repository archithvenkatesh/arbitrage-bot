...
const { KALSHI_INDEX_PATH, ensureDbDir } = require('../db');
...
async function getOrCreateIndex() {
    ensureDbDir();
    const index = new LocalIndex(KALSHI_INDEX_PATH);
...
async function indexMarkets(markets) {
    ...
    // Sort by volume and take top K (limit from config)
    const limit = require('../../config').vector.limit || 5000;
    const topMarkets = markets
        .sort((a, b) => (b.volume24h || 0) - (a.volume24h || 0))
        .slice(0, limit);

    console.log(`\n🧠 Vectorizing top ${topMarkets.length} Kalshi markets (out of ${markets.length})...`);
    ...
    console.log(`\n✅ Indexed ${indexed} Kalshi markets into vector DB`);
    return indexed;
}
